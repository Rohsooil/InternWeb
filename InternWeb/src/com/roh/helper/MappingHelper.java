...
public class MappingHelper {
	...
	public Vote getVote() throws InputInvalidException, DateInvalidException {
		...
		String title = validator.getSafeString("투표 제목", parameterMap.get("vote_title")[0], StringValidator.MAX_TITLE_LENGTH);
		String type = validator.getSafeString("투표 유형", parameterMap.get("vote_type")[0], StringValidator.MAX_TYPE_LEGTH);
	...
	public Admin getAdmin() throws InputInvalidException {
		...
		String type = validator.getSafeString("관리자 구분", parameterMap.get("admin_type")[0], StringValidator.MAX_TYPE_LEGTH);
		String name = validator.getSafeString("관리자 이름", parameterMap.get("admin_name")[0], StringValidator.MAX_NAME_LEGTH);
		String position = validator.getSafeString("관리자 직위", parameterMap.get("admin_position")[0], StringValidator.MAX_TYPE_LEGTH);
	...
	public Apartment getApartment() throws InputInvalidException {
		...
		String apartName = validator.getSafeString("아파트 명", parameterMap.get("apart_name")[0], StringValidator.MAX_NAME_LEGTH);
		String apartType = validator.getSafeString("아파트 구분", parameterMap.get("apart_type")[0], StringValidator.MAX_TYPE_LEGTH);
		String registerNum = validator.getSafeString("사업자 등록번호", parameterMap.get("apart_registerNum")[0], StringValidator.MAX_TYPE_LEGTH);
		String repType = validator.getSafeString("대표자 구분", parameterMap.get("apart_repType")[0], StringValidator.MAX_TYPE_LEGTH);
		String repName = validator.getSafeString("대표자 성명", parameterMap.get("apart_repName")[0], StringValidator.MAX_NAME_LEGTH);
		...
		String address = validator.getSafeString("아파트 소재지", parameterMap.get("apart_address")[0], StringValidator.MAX_ADDRESS_LEGTH);
	...
	public FilePath getFilePath() throws InputInvalidException {
		...
		String regisNumCardPath = validator.getSafeString("고유번호증", parameterMap.get("file_regisNumCard")[0], StringValidator.MAX_FILE_LEGTH);
		String baseDocPath = validator.getSafeString("투표신청 근거서류", parameterMap.get("file_baseDoc")[0], StringValidator.MAX_FILE_LEGTH);
		String managerCertifyPath = validator.getSafeString("관리소장 직인 및 배치신고 증명서", parameterMap.get("file_managerCertify")[0],
				StringValidator.MAX_FILE_LEGTH);
		String persAgreementPath = validator.getSafeString("개인정보수집 동의서", parameterMap.get("file_persAgreement")[0], StringValidator.MAX_FILE_LEGTH);
		String usageAgreementPath = validator.getSafeString("이용협약서", parameterMap.get("file_usageAgreement")[0], StringValidator.MAX_FILE_LEGTH);
